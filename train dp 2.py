...
def apply_gradients(dp_optimizer, gradients, variables):
    for grad, var in zip(gradients, variables):
        if isinstance(grad, tuple):
            # Handle cases where the gradient is a tuple
            #print(f"Gradient for variable {var.name} is a tuple with {len(grad)} elements.")
            for sub_grad in grad:
                if isinstance(sub_grad, tf.Tensor):
                    dp_optimizer.apply_gradients([(sub_grad, var)])
        else:
            # Handle standard tensor gradient
            dp_optimizer.apply_gradients([(grad, var)])
...
def train_with_dpsgd(source_imgs, save_path, EPOCHS_set, BATCH_SIZE,  noise_multiplier, l2_norm_clip, logging_period=1):
    ...
    def train_G(VIS_batch, ir_batch):
        ...
        apply_gradients(dp_optimizer, gradients, G.trainable_variables)
    ...
    def train_D1(VIS_batch, ir_batch):
        ...
        apply_gradients(dp_optimizer, gradients, D1.trainable_variables)
    ...
    def train_D2(VIS_batch, ir_batch):
        with tf.GradientTape() as tape:
            ...
            #print(f"D2 input shape: {generated_img_ds.shape}")
        ...
        apply_gradients(dp_optimizer, gradients, D2.trainable_variables)
    ...
    for epoch in range(EPOCHS):
        ...
        for batch in range(n_batches):
          ...
          if batch % 2 == 0:
              while tf.reduce_any(d1_loss > 1.9) and it_d1 < 20:
                  d1_loss, _, _ = train_D1(VIS_batch, ir_batch)
                  it_d1 += 1
              while tf.reduce_any(d2_loss > 1.9) and it_d2 < 20:
                  d2_loss, _, _ = train_D2(VIS_batch, ir_batch)
                  it_d2 += 1
          else:
              while (tf.reduce_any(d1_loss < 1) or tf.reduce_any(d2_loss < 1)) and it_g < 20:
                  g_loss_GAN = train_G_GAN(VIS_batch, ir_batch)
                  g_loss, g_loss_GAN_d1, g_loss_GAN_d2, d1_fake, d2_fake, generated_img = train_G(VIS_batch, ir_batch)
                  it_g += 1
              while tf.reduce_any(g_loss > 200) and it_g < 20:
